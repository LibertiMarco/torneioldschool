...
function scoreBoard(ctx,y){const home=upper($('ftHome').value,'CASA'),away=upper($('ftAway').value,'OSPITE');ctx.shadowColor='#000';ctx.shadowBlur=20;roundedRect(ctx,55,y,970,225,8,'#05080df2');ctx.shadowBlur=0;ctx.fillStyle=GOLD;ctx.fillRect(55,y,970,9);const homeX=contain(ctx,imageState.ftHomeLogo||imageState.brand,75,y+28,165,150),awayX=contain(ctx,imageState.ftAwayLogo||imageState.brand,840,y+28,165,150);ctx.fillStyle='#fff';ctx.textAlign='center';ctx.font='900 112px Impact, Arial';ctx.fillText(`${$('ftHomeScore').value||0} - ${$('ftAwayScore').value||0}`,W/2,y+137);fitText(ctx,home,245,27,15,900);ctx.fillText(home,homeX,y+205,245);fitText(ctx,away,245,27,15,900);ctx.fillText(away,awayX,y+205,245)}
