...
let TOURNAMENT_STYLE={motif:0,variant:0,monogram:'OS'};
...
function drawFulltime(){const selected=matches.find(item=>String(item.id)===String($('ftMatch').value))||matches.find(item=>String(item.torneo)===$('ftTournamentSelect').value);applyTournamentTheme(selected?.torneo_id??$('ftTournamentSelect').value??$('ftTournament').value,$('ftTournament').value);const c=$('fulltimeCanvas'),ctx=c.getContext('2d');background(ctx);brandHeader(ctx,$('ftTournament').value,'FULL TIME');const photo={x:48,y:205,w:984,h:720};if(imageState.ftCaptains){ctx.save();ctx.beginPath();ctx.roundRect(photo.x,photo.y,photo.w,photo.h,22);ctx.clip();cover(ctx,imageState.ftCaptains,photo.x,photo.y,photo.w,photo.h,cropValues('ft'));const g=ctx.createLinearGradient(0,photo.y+350,0,photo.y+photo.h);g.addColorStop(0,'transparent');g.addColorStop(1,BG+'ee');ctx.fillStyle=g;ctx.fillRect(photo.x,photo.y,photo.w,photo.h);ctx.restore();}else placeholder(ctx,photo.x,photo.y,photo.w,photo.h,'CARICA LA FOTO DEI DUE CAPITANI');
