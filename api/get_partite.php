...
try {
  ...
  while($row=$r->fetch_assoc()){
    $record = [
      ...
      "decisa_rigori"=> isset($row['decisa_rigori']) ? (int)$row['decisa_rigori'] : 0,
      "rigori_casa"=> array_key_exists('rigori_casa', $row) && $row['rigori_casa'] !== null ? (int)$row['rigori_casa'] : null,
      "rigori_ospite"=> array_key_exists('rigori_ospite', $row) && $row['rigori_ospite'] !== null ? (int)$row['rigori_ospite'] : null,
      "arbitro"=>$row['arbitro'] ?? '',
