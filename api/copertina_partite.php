...
function tournamentNameBox(ctx,x,y,w,h,text,color){ctx.fillStyle='#fff';ctx.beginPath();ctx.roundRect(x,y,w,h,10);ctx.fill();const words=upper(text,'TORNEO').split(/\s+/),lines=[];let line='';words.forEach(word=>{const candidate=line?line+' '+word:word;if(candidate.length>18&&line){lines.push(line);line=word}else line=candidate});if(line)lines.push(line);const visible=lines.length>2?[lines[0],lines.slice(1).join(' ')]:lines;ctx.fillStyle=color;ctx.textAlign='center';visible.forEach((value,index)=>{fit(ctx,value,w-28,visible.length>1?31:37,15,900);const lineGap=visible.length>1?38:0;ctx.fillText(value,x+w/2,y+h/2-lineGap/2+index*lineGap+10,w-28)})}
...
function drawReel(){const c=$('reelCanvas'),ctx=c.getContext('2d'),color=theme();base(ctx,c.width,c.height);ctx.fillStyle='#fff';ctx.textAlign='center';ctx.font='900 108px Georgia';ctx.fillText('HIGHLIGHTS',540,185);contain(ctx,homeLogo||brand,55,245,245,280);contain(ctx,awayLogo||brand,780,245,245,280);ctx.font='400 174px Arial';ctx.strokeStyle='#fff';ctx.lineWidth=5;ctx.strokeText(`${score('home')} - ${score('away')}`,540,455);ctx.save();ctx.beginPath();ctx.roundRect(38,620,1004,845,48);ctx.clip();photo?cover(ctx,photo,38,620,1004,845):placeholder(ctx,38,620,1004,845);ctx.restore();ctx.strokeStyle='#fff';ctx.lineWidth=7;ctx.beginPath();ctx.roundRect(38,620,1004,845,48);ctx.stroke();ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(285,1605,98,0,Math.PI*2);ctx.fill();contain(ctx,brand,197,1517,176,176);tournamentNameBox(ctx,505,1515,500,190,selected?.torneo_nome,color);ctx.fillStyle='#fff';ctx.textAlign='center';ctx.font='900 82px Arial';fit(ctx,upper(selected?.torneo_categoria,'CALCIO'),900,82,44,900);ctx.fillText(upper(selected?.torneo_categoria,'CALCIO'),540,1850,900)}
function drawYoutube(){const c=$('youtubeCanvas'),ctx=c.getContext('2d'),color=theme();base(ctx,c.width,c.height);ctx.save();ctx.beginPath();ctx.moveTo(835,0);ctx.lineTo(1280,0);ctx.lineTo(1280,720);ctx.lineTo(590,720);ctx.closePath();ctx.clip();photo?cover(ctx,photo,565,0,715,720):placeholder(ctx,565,0,715,720);ctx.restore();ctx.fillStyle='#fff';ctx.textAlign='left';ctx.font='900 72px Georgia';ctx.fillText('HIGHLIGHTS',55,95);contain(ctx,homeLogo||brand,45,175,185,185);contain(ctx,awayLogo||brand,45,385,185,185);ctx.font='400 170px Arial';ctx.strokeStyle='#fff';ctx.lineWidth=5;ctx.strokeText(String(score('home')),275,335);ctx.strokeText(String(score('away')),275,555);tournamentNameBox(ctx,1035,18,225,112,selected?.torneo_nome,color);ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(690,370,75,0,Math.PI*2);ctx.fill();contain(ctx,brand,624,304,132,132);ctx.fillStyle='#fff';ctx.textAlign='left';ctx.font='900 48px Arial';fit(ctx,upper(selected?.torneo_categoria,'CALCIO'),500,48,28,900);ctx.fillText(upper(selected?.torneo_categoria,'CALCIO'),70,675,500)}
