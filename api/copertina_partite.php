...
function tournamentMiniLogo(ctx,x,y,size,text,color){const name=upper(text,'TORNEO'),words=name.split(/\s+/).filter(word=>word.length>2&&!['DEL','DELLA','DI'].includes(word)),initials=(words.slice(0,3).map(word=>word[0]).join('')||'T').slice(0,3);let hash=2166136261;for(let i=0;i<name.length;i++){hash^=name.charCodeAt(i);hash=Math.imul(hash,16777619)}const variant=Math.abs(hash)%3,pad=size*.1;ctx.save();ctx.fillStyle='#fff';ctx.beginPath();ctx.roundRect(x,y,size,size,size*.07);ctx.fill();ctx.strokeStyle=color;ctx.lineWidth=Math.max(3,size*.025);ctx.stroke();ctx.translate(x+size/2,y+size*.43);ctx.strokeStyle=color;ctx.fillStyle=color;if(variant===0){ctx.beginPath();ctx.arc(0,0,size*.27,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.arc(0,0,size*.19,0,Math.PI*2);ctx.stroke()}else if(variant===1){ctx.rotate(Math.PI/4);ctx.strokeRect(-size*.22,-size*.22,size*.44,size*.44);ctx.rotate(-Math.PI/4)}else{ctx.beginPath();ctx.moveTo(0,-size*.28);ctx.lineTo(size*.27,size*.22);ctx.lineTo(-size*.27,size*.22);ctx.closePath();ctx.stroke()}ctx.textAlign='center';ctx.textBaseline='middle';fit(ctx,initials,size*.42,size*.22,size*.13,900);ctx.fillText(initials,0,0,size*.42);ctx.restore();ctx.fillStyle=color;ctx.textAlign='center';ctx.textBaseline='alphabetic';const shortName=words.slice(0,2).join(' ');fit(ctx,shortName,size-pad*2,size*.095,size*.055,800);ctx.fillText(shortName,x+size/2,y+size*.88,size-pad*2)}
...
function clearSelected(){selected=null;$('scoreFields').hidden=true;homeLogo=null;awayLogo=null;draw()}
...
async function chooseMatch(){selected=matches.find(m=>String(m.id)===$('match').value)||null;[homeLogo,awayLogo]=await Promise.all([loadImage(selected?.logo_casa),loadImage(selected?.logo_ospite)]);$('scoreFields').hidden=!selected;if(selected){const finished=Number(selected.giocata)===1;$('scoreHome').value=selected.gol_casa??0;$('scoreAway').value=selected.gol_ospite??0;$('scoreHome').readOnly=finished;$('scoreAway').readOnly=finished;$('scoreNote').textContent=finished?'Partita terminata: il risultato è bloccato.':'Partita non terminata: puoi modificare i gol solamente per questa copertina.'}$('status').textContent=selected?'Partita e scudetti caricati. Ora inserisci la foto dei capitani.':'';draw()}
