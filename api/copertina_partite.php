...
function drawReel(){const c=$('reelCanvas'),ctx=c.getContext('2d');base(ctx,c.width,c.height);ctx.fillStyle='#fff';ctx.textAlign='center';ctx.font='900 108px Georgia';ctx.fillText('HIGHLIGHTS',540,185);contain(ctx,homeLogo||brand,55,245,245,280);contain(ctx,awayLogo||brand,780,245,245,280);ctx.font='400 174px Arial';ctx.strokeStyle='#fff';ctx.lineWidth=5;ctx.strokeText(`${selected?.gol_casa??0} - ${selected?.gol_ospite??0}`,540,455);ctx.save();ctx.beginPath();ctx.roundRect(38,620,1004,845,48);ctx.clip();photo?cover(ctx,photo,38,620,1004,845):placeholder(ctx,38,620,1004,845);ctx.restore();ctx.strokeStyle='#fff';ctx.lineWidth=7;ctx.beginPath();ctx.roundRect(38,620,1004,845,48);ctx.stroke();ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(330,1605,98,0,Math.PI*2);ctx.fill();contain(ctx,brand,242,1517,176,176);ctx.fillStyle='#fff';ctx.fillRect(610,1510,205,205);contain(ctx,tournamentLogo||brand,625,1525,175,175);ctx.fillStyle='#fff';ctx.textAlign='center';ctx.font='900 82px Arial';fit(ctx,upper(selected?.torneo_categoria,'CALCIO'),900,82,44,900);ctx.fillText(upper(selected?.torneo_categoria,'CALCIO'),540,1850,900)}
function drawYoutube(){const c=$('youtubeCanvas'),ctx=c.getContext('2d');base(ctx,c.width,c.height);ctx.save();ctx.beginPath();ctx.moveTo(835,0);ctx.lineTo(1280,0);ctx.lineTo(1280,720);ctx.lineTo(590,720);ctx.closePath();ctx.clip();photo?cover(ctx,photo,565,0,715,720):placeholder(ctx,565,0,715,720);ctx.restore();ctx.fillStyle='#fff';ctx.textAlign='left';ctx.font='900 72px Georgia';ctx.fillText('HIGHLIGHTS',55,95);contain(ctx,homeLogo||brand,45,175,185,185);contain(ctx,awayLogo||brand,45,385,185,185);ctx.font='400 170px Arial';ctx.strokeStyle='#fff';ctx.lineWidth=5;ctx.strokeText(String(selected?.gol_casa??0),275,335);ctx.strokeText(String(selected?.gol_ospite??0),275,555);ctx.fillStyle='#fff';ctx.fillRect(1180,0,100,125);contain(ctx,tournamentLogo||brand,1188,7,84,110);ctx.beginPath();ctx.arc(690,370,75,0,Math.PI*2);ctx.fill();contain(ctx,brand,624,304,132,132);ctx.fillStyle='#fff';ctx.textAlign='left';ctx.font='900 48px Arial';fit(ctx,upper(selected?.torneo_categoria,'CALCIO'),500,48,28,900);ctx.fillText(upper(selected?.torneo_categoria,'CALCIO'),70,675,500)}
...
async function chooseMatch(){selected=matches.find(m=>String(m.id)===$('match').value)||null;[homeLogo,awayLogo,tournamentLogo]=await Promise.all([loadImage(selected?.logo_casa),loadImage(selected?.logo_ospite),loadImage(selected?.torneo_img)]);$('status').textContent=selected?'Partita, scudetti e torneo caricati. Ora inserisci la foto dei capitani.':'';draw()}
