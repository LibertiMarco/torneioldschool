...
try {
    if (isset($conn) && $conn instanceof mysqli) {
        ...
        $selectSql = $hasTorneoSection
            ? "SELECT config, sezione FROM tornei WHERE filetorneo = ? LIMIT 1"
            : "SELECT config FROM tornei WHERE filetorneo = ? LIMIT 1";
        if ($stmt = $conn->prepare($selectSql)) {
            ...
            if ($stmt->execute()) {
                ...
                $row = $res->fetch_assoc();
                if ($row) {
                    $torneoSection = strtolower(trim((string)($row['sezione'] ?? 'calcio'))) === 'esport' ? 'esport' : 'calcio';
                    $isEsportTournament = $torneoSection === 'esport';
                    if (!empty($row['config'])) {
